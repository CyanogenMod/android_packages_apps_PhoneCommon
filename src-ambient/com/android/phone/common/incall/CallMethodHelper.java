...
public class CallMethodHelper extends AmbientDataSubscription<InCallApi, CallMethodInfo> {
    ...
    protected void requestedModInfo(ArrayList<TypedPendingResult> queries,
        ...
        TypedPendingResult creditQuery = InCallQueries.getNudgeConfig(context, componentName,
                NudgeKey.INCALL_CREDIT_NUDGE);
        if (creditQuery != null) {
    ...
    @Override
    public void enableListeners(Context context, CallMethodInfo cn) {
        DataListeners.enableCreditListener(context, cn);
        DataListeners.enableAuthListener(context, cn);
    }

    @Override
    public void disableListeners(Context context, CallMethodInfo cn) {
        DataListeners.disableCreditListener(context, cn);
        DataListeners.disableAuthListener(context, cn);
    }

    @Override
    public void onPostResult(CallMethodInfo cmi, ComponentName componentName, Result r, int type) {
        if (r instanceof InCallProviderInfoResult) {
            InCallResults.gotGeneralInfo(cmi, componentName, mContext, (InCallProviderInfoResult)r);
