...
public class ContactsDataSubscription extends DialerDataSubscription {
    ...
    public void onDynamicRefreshRequested(ArrayList<TypedPendingResult> queries,
        ...
        queries.add(InCallQueries.getCallMethodAccountHandle(get(mContext), componentName));
    }

    @Override
    protected void enableListeners(CallMethodInfo mod) {
        InCallListeners.enableAuthListener(get(mContext), mod);
    }

    @Override
    protected void disableListeners(CallMethodInfo mod) {
        InCallListeners.disableAuthListener(get(mContext), mod);
    }
    ...
    protected void requestedModInfo(ArrayList<TypedPendingResult> queries,
        ...
        queries.add(InCallQueries.getDefaultDirectorySearchIntent(get(mContext), componentName));
        queries.add(InCallQueries.getCallMethodImMimeType(get(mContext), componentName));
